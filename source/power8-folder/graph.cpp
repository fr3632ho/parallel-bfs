...
auto thread_func(void* args) -> void*
{
  ...
  while (1) {
    ...
    data[thread_id].assign_bag->display_bag_in_binary();
    ...
    for (int i = 0; i < data[thread_id].assign_bag->size; i++) {
      ...
      for (auto& u : *temp) {
        for (auto& v : graph->adj[u]) {
          int curr_dist = dist_bag_parallel[u].load(std::memory_order_acquire);
          int next_dist = dist_bag_parallel[v].load(std::memory_order_acquire);

          if (next_dist == inf_check) {
            next_dist = curr_dist + 1;
            success_cas = dist_bag_parallel[v].compare_exchange_strong(
                inf_check, next_dist);
            if (success_cas) {
    ...
    if (data[thread_id].merge_bag->number_of_elements > 0) {
      ...
      // LOGF("merge updated by thread %d\n", thread_id);
    }
...
auto Graph::BFS_bag(const size_t number_of_threads) -> int
{
  ...
  for (int i = 0; i < number_of_threads; i++) {
    ...
    data[i].assign_bag = new bag_t(V + 1);
    data[i].merge_bag = new bag_t(V + 1);
  ...
  // exit(0);
  for (int index = 0; index < number_of_threads; index++) {
    delete data[index].assign_bag;
    delete data[index].merge_bag;
  }
